...
- **[LUA Print Interceptor]**: Injected a global `print` interceptor in `balatrobot.lua` (via `scripts/setup_mods.py`) that filters out and suppresses `"LONG DT"` prints. This eliminates excessive console/disk log spam (over 14,000 lines in 60s) caused by the gamespeed hack's large delta time (`49.9 / 60.0` in headless mode).
- **[Dynamic Polling & Timeout Increase]**: Extended the boot health check timeout to 120 seconds in `scripts/diagnose.py`. Replaced the static `time.sleep(60)` in `run_training.py` with a dynamic 2-second interval polling loop (capped at 120s) that monitors early process exits and starts training as soon as all instances are healthy.
